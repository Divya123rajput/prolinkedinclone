...
export default function ViewProfilePage({ userProfile }) {
  ...
  const getUsersPost = async () => {
    try {
      ...
      console.error("Failed to fetch posts or connections:", error);
    } finally {
  ...
  useEffect(() => {
    getUsersPost();
  }, [router.query.username]); // Fetch data when username changes

  useEffect(() => {
    const posts = postReducer.posts.filter((post) => post.userId.username === router.query.username);
    setUserPosts(posts);
  }, [postReducer.posts, router.query.username]); // Update userPosts when posts change or username changes

  useEffect(() => {
    const isConnected = authState.connections.some(user => user.connectionId._id === userProfile.userId._id);
    setIsCurrentUserInConnection(isConnected);
  }, [authState.connections, userProfile.userId._id]); // Check connections when they change
